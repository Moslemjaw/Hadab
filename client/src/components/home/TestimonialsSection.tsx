    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-cream-100/50">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col items-center text-center mb-14">
          <ThreadKnot
            color="brown"
            label={language === 'ar' ? 'الغرزة السادسة • في تفاصيل الحياة' : 'STITCH VI • IN EVERYDAY LIFE'}
            className="mb-6"
          />
          <h2 className="font-serif text-3xl sm:text-4xl text-brown-800 font-normal">
            {language === 'ar' ? 'رفيقة أصدقاء هَدَب اليومية' : 'Carried by friends of HADAB'}
          </h2>
          <p className="mt-2 text-brown-500 max-w-md font-light text-base">
            {language === 'ar'
              ? 'كلمات وانطباعات من يقتنون حقائبنا وقطعنا المنسوجة بحب.'
            ...
            >
              <div className="mb-6">
                ...
                <p className="font-serif italic text-base sm:text-lg text-brown-700 leading-relaxed">
                  &ldquo;{language === 'ar' ? (item.quoteArabic || item.quote) : item.quote}&rdquo;
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-brown-200/60">
                <img
                  src={item.image}
                  alt={language === 'ar' ? (item.authorArabic || item.author) : item.author}
                  className="w-10 h-10 rounded-full object-cover border border-brown-300"
                />
                <div>
                  <div className="text-xs font-semibold text-brown-800">
                    {language === 'ar' ? (item.authorArabic || item.author) : item.author}
                  </div>
                  <div className="text-[11px] text-brown-400">
                    {language === 'ar' ? (item.locationArabic || item.location) : item.location} • <span className="text-burgundy-500 font-medium">{language === 'ar' ? (item.pieceNameArabic || item.pieceName) : item.pieceName}</span>
                  </div>
                </div>
              </div>
            </div>
